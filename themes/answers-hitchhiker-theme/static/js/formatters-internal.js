...
//reopen status for hours
export function statusa(profile) {
  if (profile.hours && profile.hours.reopenDate) {
    if(profile.additionalHoursText){
     return profile.additionalHoursText
    }
    else{
...
//reopen status for deliveryHours
export function statusb(profile) {
  if (profile.deliveryHours && profile.deliveryHours.reopenDate) {
    if(profile.c_additionalDeliveryHoursText){
     return profile.c_additionalDeliveryHoursText
    }
    else{
    return 'Temporary Unavailable for Delivery Services';
    }
...
//reopen status for takeoutHours
export function statusc(profile) {
  if (profile.takeoutHours && profile.takeoutHours.reopenDate) {
    if(profile.c_additionalTakeawayHoursText){
     return profile.c_additionalTakeawayHoursText
    }
    else{
    return 'Temporary  Unavailable for Takeaway Services';
    }
  } else {
    return '';
  }
...
export function getDirectionsUrl(profile, key = 'address') {
  ...
  let url = `https://www.google.com/maps/dir/?api=1&destination=${query}&output=classic`;
  navigator.geolocation.getCurrentPosition(function (position) {
    console.log(position.coordinate.latitude, position.coordinate.longitude)
  })
  if (profile.googlePlaceId) {
    url += `&query_place_id=${profile.googlePlaceId}`;
    //https://www.google.com/maps/dir/?api=1&origin=${lat},${lng}&destination=${query}&output=classic
    //href="https://www.google.com/maps/dir/?api=1&origin=' + lat + ',' + lng + '&destination=' + location.address.line1 + ' ">
  }
