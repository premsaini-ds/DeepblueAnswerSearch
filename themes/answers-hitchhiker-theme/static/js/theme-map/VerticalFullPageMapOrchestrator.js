...
class VerticalFullPageMapOrchestrator extends ANSWERS.Component {
  ...
  pinFocusListener (index, cardId) {
    ...
    if (this.isMobile()) {
      ...
      this.scrollToResult(card,index);   
  ...
  scrollToResult(targetEl,index) {
    ...
    if (!isViewableWithinContainer(targetEl, scrollContainer,)) {
      $(".mCSB_container").css("top", `-${scrollDistance}px`);
      const newIndex = index-1;
      const draggerDistance = newIndex*30
      $(".mCSB_dragger").css("top", `${draggerDistance}px`);
      smoothScroll(scrollContainer, scrollDistance, 400,index);
    }
  }
